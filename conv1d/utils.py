...
def kfcv_fit2(builder, x, y,
    ...
    for index, (train, val) in enumerate(kfold.split(x[0], np.argmax(y, axis=-1))):
        ...
        if noise_std != None:
            x_train[0] = np.r_[x_train[0], data_enhance_noise(noise_std, x_train[0])]
            x_train[1] = np.r_[x_train[1], data_enhance_noise(noise_std, x_train[1])]
            y_train = np.r_[y_train, y_train]
            x_train, y_train = shuffle(x_train, y_train)
            print('Data enhanced (%s) => %d' % ('noise', len(x_train[0])))
